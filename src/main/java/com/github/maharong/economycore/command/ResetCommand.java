...
public class ResetCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        // 인자 개수 체크
        if (args.length < 1) {
            MessageUtil.send(sender, "money.usage.reset");
            return;
        }
        ...
        // 시작 금액으로 초기화
        FileConfiguration config = EconomyCore.getInstance().getConfig();
        double amount = config.getDouble("starting-money", 100.0);
        EconomyManager.set(target, amount);
        MessageUtil.send(sender,"money.reset", Map.of(
